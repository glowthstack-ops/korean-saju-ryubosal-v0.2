...
// - 로그인: 현재 "선택된 사주"만 사용(선택 없으면 null). 균시차·자시 규칙은 사주별 속성.
// - 게스트: IndexedDB 로컬 프로필 + 기기 로컬 균시차·자시 규칙.
...
import { loadEotPreference, loadJaHourRulePreference, loadProfile } from "@/lib/storage";
import {
...
import type { JaHourRule, Profile } from "@/lib/types";

/** 오버레이/일주 계산 기준 사주 해석 (CalendarGrid와 공유). */
export async function resolveOverlayProfile(): Promise<
  { profile: Profile; eot: boolean; jaHourRule: JaHourRule } | null
> {
  if (isLoggedIn()) {
    ...
    try {
      ...
      return {
        profile: summaryToProfile(summary),
        eot: subjectEotPreference(summary),
        jaHourRule: subjectJaHourRule(summary),
      };
    } catch {
      return null;
    }
  ...
  return profile
    ? { profile, eot: loadEotPreference(), jaHourRule: loadJaHourRulePreference() }
    : null;
}
...
export async function resolveCurrentSubjectIlju(): Promise<string | null> {
  ...
  try {
    const result = await calculateManse(
      resolved.profile,
      undefined,
      buildTimeOptions(resolved.eot, resolved.jaHourRule),
    );
