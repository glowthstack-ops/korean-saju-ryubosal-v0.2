...
#: B13 감정 키워드. 활용형을 **열거**한다 — `힘들[어다]` 만 두면 "힘들지·힘들네" 를 놓치고,
#: 반대로 어간 `힘들` 을 그대로 두면 과거형 서사를 삼킨다.
#:
#:     "작년에 왜 그렇게 힘들었을까?"        Q5 과거 사건 설명이지 감정 토로가 아니다
#:     "…힘들때가 있었는데 언제인지 맞춰봐"   시점 되짚기(past validation)다
#:
...
#:
#: `왜 이렇게` 단독은 넣지 않는다 — "지금 대운은 왜 이렇게 흘러가?" 처럼 흐름 **설명
#: 요구**와 구분되지 않아 정상 분석 질문을 삼킨다(패턴 매트릭스 실측). 부정 표현이
#: 뒤따르는 형태만 잡되, 어순이 양쪽 다 나오므로 두 갈래로 본다.
#:
#:     왜 → 나    "왜 나는 되는 일이 없을까"
#:     나 → 왜    "나는 왜 이렇게 운이 안 좋지"
_LAMENT_NEGATIVE = r"(?:없|안\s*좋|못하|안\s*되|안\s*풀)"
_LAMENT = re.compile(
    rf"왜\s*(?:나|내|저)\s*(?:는|만|한테|에게)?[^?]*?{_LAMENT_NEGATIVE}|"
    rf"(?:나|내|저)\s*(?:는|만|한테|에게)[^?]*?왜[^?]*?{_LAMENT_NEGATIVE}|"
...
RECIPROCAL_PREDICATE_RE = re.compile(
    ...
    # 종결형만 잡으면 우회 어미를 놓친다 — '잘될까'는 걸리는데 '잘될 수 있을까'가 빠져
    # 같은 질문이 too_broad 로 떨어졌다(실로그 '남자1과 잘될 수 있을까?').
    r"|잘\s*될?\s*(?:까|수|지|는지|런지|것)"
    r"|잘\s*풀릴|잘\s*지낼"
    r"|돌아올까|돌아와|돌아올\s*수"
    r"|결혼(?:할\s*수|하게|까지)"
    r"|재결합|복연|복합"
...
def _detect_query_type(text: str, subjects_mode: SubjectMode) -> QueryType:
    ...
    # Q13 — 감정 토로 (B13).
    #
    # 물음표로 가르지 않는다. docs/08 B13 은 "질문 없이 서사만 있는 경우 **포함**" 이라
    # 질문이 있으면 B13 이 아니라는 뜻이 아니다. 실제로 물음표 하나 때문에
    # "너무 스트레스야 어떻게 해야 할까?" 가 too_broad(범위 좁히기 메뉴)로 빠졌다.
    #
    # 한탄·자조 어법은 **도메인·명리 용어가 없을 때만** B13 으로 본다.
    #   "왜 나는 연애운이 없을까"      사용자가 이미 분야를 지목 → domain_analysis 유지
    #   "내 대운은 왜 이렇게 안 좋아?"  명식 구조 질문 → 공감으로 대체하면 안 된다
    # B13 파생 규칙도 "도메인 연결은 사용자가 원할 때만" 이다.
    if _EMOTION_WORDS.search(text) or (
        _LAMENT.search(text)
