...
# 2026-09-18: 체감→관찰 가능 사건 번역·12운성 가드(데굴님 지시)
PROMPT_REVISION = "2026-09-18.observable-events"
...
_SYSTEM = (
    ...
    "물음표·느낌표·가운뎃점만 쓴다.\n"
    # 9) 사건 어휘(PROMPT_REVISION 2026-09-18 데굴님 승인 — 전문가 참고 기준 B3): 체감 표현은
    # 관찰 가능한 사건으로, 좋은 일은 '돈·합격'만이 아니라 회복·해소·부담 경감까지, 나쁜 일은
    # 손실 확정 대신 '문제의 종류'까지만. 12운성 이름(병·사·절)을 질병·죽음·단절로 옮기지 않는다.
    "9) 사건 어휘: '운이 좋다·나쁘다', '압박받는다', '실속이 없다' 같은 체감 표현은 원문 의미를 "
    "지키는 범위에서 눈에 보이는 일(연락이 온다, 승인이 난다, 정산이 미뤄진다, 비용이 는다)로 바꿔 "
    "쓴다. 좋은 일은 돈이나 합격만이 아니라 풀림·회복·부담이 줄어드는 일도 포함한다. 나쁜 일은 "
    "'잃는다·빼앗긴다'로 못 박지 말고 경쟁이 생긴다, 미뤄진다, 얽힌다까지만 쓴다. 병·죽음·이별 "
    "같은 단정 표현은 원문에 없으면 넣지 않는다.\n"
)
