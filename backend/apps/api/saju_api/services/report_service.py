...
_MONTH_FLOW_GUIDE_TAIL = (
    ...
    "(가장 도움되는) 달로 짚고, 각 달 기운의 활용·대비 방향도 곁들일 것. 좋은/주의 라벨은 "
    "표의 등급·⚠ 마커가 실제로 있는 달에만 붙인다 — 표에 없는 주의 달이나 좋은 달을 균형을 "
    "위해 만들어 넣지 말 것."
)
...
# 저작 지시 누출 줄(2026-08-14) — 부록 점수표의 verbatim 복사 지시에 딸려 LLM이 표 위
# '[점수표 …]' 라벨 줄이나 구 캡션 줄('(신호 강도는 좋고 나쁨이 아니라 …)')을 본문에
# 그대로 복사한 경우 결정적으로 제거한다(재생성 없이 후처리 안전망).
_AUTHORING_LEAK_LINE = re.compile(
    r"(?m)^[ \t]*(?:\[점수표[^\]\n]*\]|\(신호 강도는 좋고 나쁨이 아니라[^)\n]*\))[ \t]*$"
)
...
def _tighten(text: str) -> str:
    ...
    줄과 점수표 저작 지시 줄('[점수표 …]'·구 강도 캡션)이 본문에 그대로 노출된 경우
    (전문용어·지시문 누출) 결정적으로 제거한다.
    """
    text = _EVIDENCE_LINE.sub("", text)
    text = _AUTHORING_LEAK_LINE.sub("", text)
    text = _TRAIL_WS.sub("\n", text)
...
# 부록 점수표 공통 가이드(W-09·J-08·R-08·RP-10) — 표 verbatim 유지 + 내부 표기 비노출.
# 2026-08-14 누출 수정: 표 위 '[점수표]' 라벨 등 대괄호 내부 표기는 본문 복사를 금지하고,
# '신호 강도=발동 강도(길흉 아님)' 안내는 표 안 캡션 대신 LLM이 자기 문장으로 풀어 쓴다
# (캡션을 표 블록에 끼우면 verbatim 지시에 따라 그대로 사용자 노출 — 실사용 리포트 실증).
_SCORE_TABLE_GUIDE = (
    "아래 점수표를 마크다운 표 형식(| ... |)과 구분선(|---|)까지 그대로 본문에 포함하라"
    "(이 부록 섹션은 평문 규칙의 예외 — 표 기호 유지). 표 안 수치·간지·방향은 한 글자도 바꾸지"
    " 말고, 표 밖에서 새 수치를 만들지 말 것. 표 위아래에 짧은 안내문만 덧붙여라. "
    "'[점수표]' 같은 대괄호 라벨 줄은 내부 표기이니 본문에 옮기지 말 것. 안내문에는 신호 "
    "강도가 좋고 나쁨이 아니라 그 주제가 얼마나 강하게 발동하는가를 뜻하며(강도가 높고 예상 "
    "방향이 부정이면 부정 쪽으로 강하게 움직인다는 뜻), 예상 방향과 함께 읽어야 한다는 점을 "
    "자기 문장으로 풀어 쓸 것."
)
_SECTION_GUIDES: dict[str, str] = {
    ...
    "'[점수표]' 같은 대괄호 라벨 줄은 내부 표기이니 본문에 옮기지 말 것.",
    # ── 재물운 테마 전용(W-01~W-09) ──
...
_HALF_YEAR_PAGE_GUIDE = (
    ...
    "기운의 활용·대비 방향을 곁들일 것. 단 좋은/주의 라벨은 표의 등급·마커가 실제로 있는 "
    "달에만 붙인다(없는 축은 만들어 넣지 말 것). 한두 강신호를 여러 달에 반복하지 말고, "
    "다른 반기의 달은 다루지 말 것."
)
...
class _ReportData:
    ...
    def year_spectrum_block(
        ...
        return [
            ...
            "(길흉=용신/기신)이며, 각 해 기운의 활용·대비 방향을 곁들일 것. 좋은/주의 "
            "라벨은 표의 등급이 실제로 있는 해에만 — 없는 축을 만들어 넣지 말 것.]",
            *spectrum,
        ]
    ...
    def decade_detail_block(self, sid: str) -> list[str]:
        ...
        if rows:
            lines += [
                ...
                "등급〈…〉(길흉=용신/기신) — 등급이 없는 축의 해를 만들어 넣지 말 것]",
                *rows,
            ]
        return lines
...
def _product_framing(spec: ReportSpec) -> str:
    ...
    if spec.product_code == "RPT_YEAR":
        ...
        return (
            ...
            "어떻게 활용·대비할지 실천 방향을 곁들인다. 단, 좋은/주의는 데이터의 등급·마커가 "
            "실제로 있는 시기에만 붙인다 — 없는 주의 시기나 좋은 시기를 균형을 위해 만들어 "
            "넣지 말 것."
        )
    ...
    return (
        ...
        "묶어 해당 기간의 흐름 중심으로 서술한다. 좋은 시기와 주의 시기를 함께 짚되, 데이터에 "
        "실제로 있는 쪽만 짚는다 — 없는 주의/기회 시기를 균형을 위해 만들어 넣지 말 것."
    )
...
def build_section_context(
    ...
    if sid in _PARTNER_NATAL_SECTIONS:
        ...
        lines.append("")
        # 중립 데이터 라벨만 — 지시문(그대로 인용·수치 생성 금지)은 _SCORE_TABLE_GUIDE가
        # 담당한다. 지시문을 표 옆에 붙이면 verbatim 복사 지시에 딸려 사용자 본문에
        # 그대로 노출된다(2026-08-14 실사용 리포트 실증 → 누출 수정).
        lines.append("[점수표]")
...
_RELATIONSHIP_BETA_REPORT_DIRECTIVE = (
    ...
    "단정·낙인 없이 활성·유지 우호도·종료압력 세 흐름만 참고해 서술한다. beta·잠정 관측 같은 "
    "내부 상태 고지 문구는 본문에 노출하지 않는다(2026-08-24 사용자 확정 — 실서비스 톤 유지)."
)
...
_RELATIONSHIP_RISK_BETA_REPORT_DIRECTIVE = (
    ...
    "강도 흐름만 예방적·차분한 톤으로 서술한다. beta·잠정 관측 같은 내부 상태 고지 문구는 "
    "본문에 노출하지 않는다(2026-08-24 사용자 확정 — 실서비스 톤 유지)."
)
