...
def chat(
    ...
    """단일 질문을 풀이한다(MVP — 첫 intent 기준, 다중 intent는 메타로 동반).

    Args:
        birth: 대상 출생 정보(현 단계 subject=요청 본문의 차트 — Subject Manager는 Phase 4).
        question: 사용자 질문 원문.
    ...
    if assessment.status in ("too_broad", "need_subject"):
        ...
        return ChatResponse(
            ...
            intents=parsed.intents, assessment=assessment,
        )
