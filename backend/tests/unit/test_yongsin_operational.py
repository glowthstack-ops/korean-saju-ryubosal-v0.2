"""용신 2계층 역할(canonical/operational) — Phase 0 + 희신 과다 교정(2026-07-12).

operational 은 선택 모델의 자체 역할맵으로 정상화. 정적 순환의 희신이 원국 과다 병이면
final 도 모델맵을 채택한다(희신=과다 병 교정 — 데굴님 확정). 부분맵·특수분기 모델은
canonical 폴백(None 역할 미발생). 규격: doc/v2_2/YONGSIN_OPERATIONAL_ROLE_SPEC.md
"""
...
def test_standard_killing_resource_operational_normalizes(make_pillars) -> None:
    ...
    # 희신=과다(병) 교정(2026-07-12 데굴님 확정) — 정적 순환의 희신 水가 원국 과다 병
    # (관살과다)이므로 final 도 선택 모델 자체맵(비겁 방조=희신)을 채택 → 희신=火·한신=水.
    ...
    # final = 모델맵 채택(희신 과다 교정 — 구 '정적 순환 불변'은 의도적으로 폐기)
    assert y.final["yongsin"] == "木"
    assert y.final["heesin"] == "火"
    assert y.final["hansin"] == "水"
    # canonical_roles 는 final 5역할의 미러
    assert y.canonical_roles == {k: y.final[k] for k in _ROLE_KEYS}

    canon, oper = _canon(y), _oper(y)
    assert canon["火"] == "희신" and canon["水"] == "한신"   # 교정된 canonical
    # 木 용신·金 기신은 phase 안정. 火·水 의 작동역할 정밀화는 Phase 1~2 테스트가 검증한다.
    assert oper["木"] == "용신" and oper["金"] == "기신"
    assert oper["火"] == "조후보조신"          # 겨울 조후 격상
    assert oper["水"] == "조건부 한신/병"      # 과다 병 — 중첩 유입 시 기신성
