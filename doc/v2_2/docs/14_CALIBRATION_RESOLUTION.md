...
## 4. 문항 UX (P2 → CAL-P3 개정 2026-09-21)

원안(3단계: ① 연도 전체 체감 → ② 영역별 → ③ 사건별)은 문항당 최대 35회 선택으로 부담이 컸다. **개정**:
③ 사건별 "결과 어땠나"(판별 주축)를 맨 앞에, ① 연도 전체 체감(7상태)을 그다음, ② 영역별(5상태+무신호)은
"더 자세히 답하기" 접힘(선택). 참고 문항(교운·평소·해당 시기·성향)은 "참고 질문 · 용신 확정에 반영되지 않아요"
배지로 구분해 판별 문항 뒤에 둔다. 채점식(§3)은 불변 — 노출 순서·접힘만 바뀐다.
...
## 9. CAL-P3 — 질문 판별력·사용자 이해도 최적화 (2026-09-21 데굴님 승인)

배경(실측 1980-11-22): 기본 5문항 중 3문항(2015·2016·2017)의 사건 기대 극성이 primary 모델 간 **전부 동일**해
답이 점수를 못 가르고(raw 18:18:18 동점 → prior confidence 로만 결정), 같은 직업 사건 4종이 4문항에 반복되며,
연속 연도(2014~2017)가 몰리고, 12·15세 해에 결혼·사업 사건을 물었다. 결과 화면은 "土/火/木/水"와 내부 용어뿐이었다.

1. **연도 선택(`question_generator`)**: `event_discrimination`(primary 모델 간 기대가 갈리는 사건 수)을 연 단위
   score 에 가산(3/건), 미성년(만 19세 미만) 해는 후순위, 이미 쓴 해의 ±1년은 후순위, q1~q5는 판별 사건>0 인 해 우선
   (대안 없을 때만 완화). 명리 규칙 추가 없음 — 기존 `expected_by_model` 재사용. event_provider 가 없으면(합성 기간)
   기존 순서 유지.
2. **사건 다양화**: 연도별 후보 8건 중 앞 문항에 덜 나온 사건 우선·카테고리당 최대 2건으로 4건 노출.
3. **문구**: "2021년(만 41세)에 아래 일이 있었다면 결과가 어땠는지 골라 주세요" + `hint`("후보 해석끼리 예측이 갈리는
   해예요…"). 간지·세운·입춘 범위는 `period_range`(작은 글씨)로. 교운기 문항 2문장.
4. **결과 설명(`result_narrator.narrate`)**: `CalibrationResult.user_summary` — ①"답하신 n개 중 m개가 '土 기운이 도움이
   된다'는 해석과 맞았어요 → 용신 土 확정" ②용희기구 역할 의미(정의 문구) ③유력/불확실이면 남은 판별 문항 안내.
   `explanation`(내부 진단)은 그대로 두고 화면은 `user_summary` 를 쓴다(판정 근거 상세는 접힘).
5. **불변**: 채점식·모델 prior·용신 산출·probe 비반영 원칙(§7)·무신호 규칙(§8) 모두 불변.
