...
월간: 핵심흐름 / 일·직업 / 재물 / 관계·연애 / 건강 / 주의시기 / 기회시기
연간: 핵심주제 / 상·하반기 / 직업 / 재물 / 관계 / 건강 / 주의시기 / 기회시기
```

> 월간 슬롯은 연간 도메인 구조를 월 스케일로 옮긴 것(2026-06-12 사용자 확정). 주의·기회 시기는 그 달 안의 주·일을 가리킨다. 종합운 점수는 운 위계 가중(대운>세운>월>일)으로 합산하되 — 대운·세운이 형성한 기운이 하위 기간에서 사건화 — 출력은 해당 기간 단위 사건·조짐으로 한정한다(단정 금지).
...
### E10-d. 계약일/이삿날 십성 분리 (R3 — 이사운 고도화)
이사·이동 목적은 **계약일과 이삿날의 좋은 십성이 다르다**. `calendar/date_selection_ten_gods.json`
점수표를 `day_execution`에 블렌드(`0.5·도메인신호 + 0.5·십성적합`)한다.

- **계약일(`contract_document`)**: 정관·정인 우대(서류·약속), 편인·겁재·상관·편관·충 회피. 가중 `천간십성 0.45 / 지지관계 0.35 / 천간오행 0.20`.
- **이삿날(`relocation`)**: 정재·정관 우대(물건·실행), 정재+정관·재관 조합 가산, 일지합 가점/일지충·월지충 감점. 가중 `지지관계 0.45 / 천간십성 0.30 / 지지십성 0.25`.
- **충 이중성**: 충은 이벤트 탐지엔 이동 동력(긍정)이나 택일엔 돌발 리스크(감점)로 처리.

전체 사양은 `doc/v2_2/RELOCATION_ENHANCEMENT.md` 참조.
