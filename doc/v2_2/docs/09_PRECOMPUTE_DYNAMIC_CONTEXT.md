...
> **v2.2.1 개정 (2026-06-12, 사용자 승인)**: 해석 사전(docs/05 interpretations/) 직렬화 도입에 따라
> 입력 상한을 상향하고 thinking을 low 이하로 허용한다. **토큰 절감을 위해 풀이 품질을 희생하지 않는다.**
> 입력 증가분의 대부분은 사용자별 고정 prefix(원국·명식 해석 자료)이며 provider 프롬프트 캐시로 흡수한다
> (Gemini implicit/explicit caching, OpenAI 자동 prompt caching — 직렬화 순서 고정이 조건, docs/06).

> **출력 상한 주의(2026-06-12)**: Gemini는 `maxOutputTokens`에 **thinking 토큰 + 가시
> 출력**이 합산된다. thinking(LOW도 ~1.2k 소모)이 가시 답변을 잠식해 답변이 잘리지
> 않도록 출력 토큰 상한을 thinking+출력 합산 기준으로 잡는다. 실제 가시 답변 길이는
> 별도 `출력 글자수` + 프롬프트 길이 지시가 강제한다.

| 호출 유형 | LLM 입력 상한 | LLM 출력 상한(thinking+가시) | 가시 답변 길이 | thinking |
|---|---|---|---|---|
| 대화형 단건 응답 | 12,000 tok (고정 prefix ~5k 포함) | 5,000 tok | 1,500자 | **low 이하** |
| 대화형 — 동반자 비교 | 14,000 tok | 5,500 tok | 2,400자 | low 이하 |
| Query Parser (경량 모델) | 2,000 tok | 300 tok | — | **비활성** (분류 작업) |
| 집중 풀이 섹션 1개 | 10,000 tok | 8,000 tok | 4,500자 | low 이하 |
| 총운 풀이 섹션 1개 | 10,000 tok | 8,000 tok | 4,500자 | low 이하 |
| 정합성 검사 LLM 패스(선택) | 8,000 tok | 500 tok | — | 비활성 |

가드 구현: LLM 호출 래퍼가 입력 토큰을 측정해 상한 초과 시 **호출 전 예외** → Context Reduction 재실행. 상한을 늘리는 코드 수정은 금지(사용자 승인 필요 — 본 v2.2.1 상향은 승인 완료). 모든 호출의 입출력 토큰을 로깅해 상품별 원가 대시보드에 집계하며, **캐시 적중 토큰을 별도 집계**해 실비용을 추적한다.
