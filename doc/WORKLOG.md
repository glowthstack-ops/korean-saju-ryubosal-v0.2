...
## 이벤트 방향·근거 무결성 P0~P4 (2026-07-22 데굴님 확정 — GPT 검토 최종안)

금전·횡재운 리포트 실사례: 표 전 행이 '횡재+손실·지출'로 모순 표기, '運 子↔원국 丁
천간합' 차단급 근거 오류, 점수순 Top-N이 부정 후보 일색(긍정 wealth_change/opportunity
13건 존재에도 컷), 근거 밖 사건(투자·독소조항·거주지 이동) 창작 + 저신뢰인데 정밀 단정.

- **P0 관계 endpoint 불변식**: _relation_hit이 모든 관계에서 luck_ref에 천간·지지를 둘 다
  채우고 렌더러가 branch 우선으로 뽑아 천간합이 지지로 표기되던 참조 오류(계산은 정상) —
  원천(천간 관계=천간만/지지 관계=지지만) + 렌더러 이중 방어. 실차트 검증 '運 壬↔원국 丁
  천간합' 정상화. 두 글자 결집 라벨 '삼합/방합(세력 보조)'→'~ 일부 결집(미완성·세력
  보조)'(완성 국 오해 차단).
- **P1 lite 결과방향 파생축**(표시·서술 전용, 점수·판정 불변): result_direction —
  positive/negative/delay/activation/unknown. pressure=경험 품질이라 activation(활성화만)
  으로 — '합격+부담'을 실패로 격하하지 않음(§3 승인). 완전한 3축 분리(성사/불성사 엔진
  판정)는 후속 과제.
- **P2 방향 인지 라벨**(감수 문안): event_display_ko — 횡재는 긍정 확정시만, 부정=
  '예상 밖 지출·손실 위험', 중립='돌발 금전 변동'. 취업/시험/승진/연애/결혼 6키 문안
  적용. 표 헤더 '이벤트→사건', '점수→신호 강도'(+강도≠길흉 설명), '방향→예상 방향'.
  리포트 표·클러스터 + 채팅 후보(context_reducer)·V2 직렬화 모두 배선.
- **P3 표 계층 선별**(select_table_candidates): 동일 사건·인접 기간 중복 제거 → 결과
  방향별 대표(존재하는 방향만 — 고정 긍정 쿼터 금지) → 점수순 충원. 본문 후보는 불변,
  부록 표만 전체 풀에서 재선별.
- **P4 근거 충실 가드**(EVIDENCE_FIDELITY_DIRECTIVE, chat+리포트 전 섹션 공용): 근거 밖
  구체 사건 창작 금지·저신뢰→구체성 하향·부정 일색은 사실대로(억지 긍정 금지, 대조는
  실제 긍정 후보 있을 때만)·신호 강도≠길흉 명시. C안(강제 긍정 대비) 폐기 반영.

회귀 test_event_direction_integrity.py 9케이스(endpoint 불변식 실차트 검증·혼합 표기
차단·미완성 라벨·파생축·모순 라벨 차단·pressure 비격하·선별 중복 제거/쿼터 금지).
pytest 2309 passed·ruff·mypy clean. 후속 보류: 사건 의미축 완전 분리(성사/불성사 엔진
판정), WINDFALL 내부 키 중립화, 관계 점수 기여값-표기 일치 감사, 공망 충발 정의 명문화.

## 보류 항목 재분류 반영 — 출시 전 필수 4건 (2026-07-22 데굴님 확정)

'전부 후속 보류' 철회 — 정확성 문제(관계 감사·공망 정의)와 표현 품질(바넘)은 즉시 처리:

1. **관계 기여 일치(P0)**: 부록 표의 관계 근거는 기간 공통 데이터인데 후보 점수 기여처럼
   보이던 착시 — 후보 signals에 '관계 발동'이 없으면 '[시기 참고 — 이 후보 점수의 직접
   근거 아님]' 마커로 구분. 감지 일치(엔진 hit 그대로)·endpoint 불변식·미완성 라벨은
   기구현, **수치 일치(관계별 delta 구조화 — §3 JSON 형식)는 엔진 확장 후속**(레거시
   후보 계층에 per-관계 delta 부재 실측).
2. **공망 충발 정의 명문화(P0)**: 엔진 실정의 확인 — 적용: 운 지지↔원국 공망 자리
   (전실/충발/해소 3종 hit), 효과: 점수 -10 + timing=DELAY(발현 지연), 방향·quality
   불변, 기간당 게이트 1회(addendum_gate_modifier). 즉 실패·무산 판정 신호가 아님 —
   '정의 후 유지' 경로: 라벨 '공망 충발(지연·변동 보조)' + GONGMANG 디렉티브에 불변식
   ('충발 단독 실패·공허 단정 금지, 독립 근거 동반 시 지연·불확실성 보조만') 추가.
3. **바넘 문장 억제(P1 저비용)**: BARNUM_SUPPRESSION_DIRECTIVE(chat+리포트 전 섹션) —
   비요청 성격 서두 금지·질문 무관 미덕 칭찬 금지·첫 문단=핵심 판정·호칭 반복 최소화.
4. **의미축 스키마 예약(단기)**: EventCandidate에 outcome/experience 예약 필드(기본
   None=미판정, 억지 채움 금지) + quality≠outcome 경고 docstring. 결과 명사는 outcome
   확인 시만 노출 원칙 명문화. 성사/불성사 판정 엔진은 후속 개발.
5. **WINDFALL 별칭(중장기 준비)**: EVENT_SEMANTIC_ALIAS(sudden_financial_change) +
   마이그레이션 요건(버전 필드·읽기 호환·통계 통합·fixture 재생성·부모-자식 구조) 주석
   명문화 — canonical 키는 유지(과거 긍정 횡재 데이터 의미 보존).
...
## 2026-08-04 — 되물음 답변이 끊기던 결함 (CHAT-OFFER-QUESTION-01)

### 관측

```
시스템  "… 절대 양보할 수 없는 한 가지는 무엇인가요?"  "※ 관계 신호는 시험(beta) 관측치예요."
사용자  "역시 외모지"
시스템  "질문 범위가 넓어요. 이렇게 좁혀볼까요? — 향후 6개월 연애운 / 올해 연애운"
```

시스템이 던진 질문에 사용자가 답했는데 **새 질문으로 파싱**해 범위 좁히기 메뉴로
바운스했다.

### 원인 — 규칙이 아니라 입력이 비어 있었다

offer-answer 링킹 규칙(2026-07-22)은 이미 있었다. 문제는 그 입력인 `last_offer` 다.

- `_extract_offer` 가 제안 어구 **키워드 목록**(`짚어드릴까요`·`궁금해요` 등)으로만
  offer 를 뽑아, 페르소나가 사용자에게 직접 묻고 끝낸 질문형 마감("… 무엇인가요?")을
  하나도 잡지 못했다 → `last_offer=''` → offer-slot·offer-answer 둘 다 건너뜀 →
  `LinkKind.NEW` → `assess()` → too_broad.
- 같은 원인으로 `_OFFER_CONTINUE_DIRECTIVE` 도 죽어 있었다(LLM 이 사용자가 무슨 질문에
  답한 것인지 모른 채 생성).

### 수정

1. **질문형 마감을 구조로 인식**(`_extract_offer`): 마지막 문장이 물음표로 끝나면 대기
   질문으로 인정. 마커 목록은 그대로 유지(회귀 없음). **마지막 문장으로 한정**한 이유는
   tail 2문장 전체에 적용하면 본문 중간 수사의문문("이 시기가 정말 좋을까요? 결론적으로
   는 …")까지 제안으로 잡히기 때문이다. 답변 말미 고지 줄(`※ …`)은 tail 계산에서 제외
   한다(고지가 붙으면 실제 질문이 마지막 문장 밖으로 밀린다).
2. **되물음 답변 전용 지시문**(`_OFFER_ANSWER_DIRECTIVE`): 기존 문구는 "짧은 수락
   ('그래/봐줘')으로 제안을 받아들였다" 로 시작해 내용 답변에 쓰면 LLM 이 제안 수락으로
   오해하고 답 내용을 버린다. 질문형 offer + 비수락 발화일 때만 답변용 문구를 쓴다.
3. **회귀 13종**(`test_offer_question_answer_link.py`): offer 추출 6종(실로그·고지 2줄·
   기존 마커 2종·평서 마감·중간 수사의문문 오탐)·링킹 3종·지시문 선택·2턴 재생 e2e.

### 검증 중 발견한 4번째 결함 — 무인증 스레드 경로의 offer 미갱신

실서버 재현이 계속 실패해 추적한 결과, 라우터에 생성 경로가 둘이었다.

- **로그인+스레드**(실서비스): 백그라운드 `_run_chat_answer` → `update_thread_offer`
  → `_extract_offer` 사용 → 수정으로 정상화.
- **비로그인+스레드**: 라우터가 답변을 직접 생성해 `chat_service` 의 offer 저장 지점
  (`state.last_offer = _extract_offer(answer)`)을 **아예 지나지 않는다** → last_offer 가
  영원히 ''. 대화 상태 자체는 prep 단계에서 이미 영속화되므로 일관성이 깨진 상태였다.

승인 범위(3항목) 밖이지만 같은 결함 계열이고 이것을 두면 무인증 스레드에서 결함이
그대로 남아 라우터에 `update_thread_offer` 호출을 추가했다(백그라운드 경로와 동일 계약).

### 실서버 재현 검증

```
1턴  "내게 앞으로 올 연애 이벤트가 있을까?"  → last_offer 저장 확인(질문형 마감)
2턴  "역시 외모지"                          → status=answered (기존: too_broad)
```
답변이 연애 맥락을 이어받았고 사용자의 답도 반영됐다("이성적인 끌림 앞에서는 뚜렷한
한 가지 강점에 매료되어 마음을 여는").
...
## 2026-08-04 — 재부팅으로 v2 인프라만 사라진 사고 (INFRA-RESTART-01)

### 관측

"서버가 날아갔다 — 이전 v1 서버밖에 노출되지 않는다"는 신고. 실제 상태는 다음과 같았다.

| 확인 | 결과 |
|---|---|
| 머신 | 20:28 재부팅(로그·터널은 20:08 정상 종료로 끊김) |
| `saju-db-1`·`saju-redis-1`(v1) | `restart: always` → 자동 복구, 5432/6379 점유 |
| `saju-v2-db`·`saju-v2-redis` | **컨테이너 레코드 자체가 없음** — 15432/16379 미노출 |
| `saju_v2_saju-v2-pgdata` | 137.8MB PGDATA 온전(마지막 쓰기 = 정상 종료 시각) |
| git worktree·HEAD | `0c361b6` 그대로, 유실 없음 |

`docker system df` 기준 엔진 전체 컨테이너가 2개(v1 쌍)뿐이었다. 중지가 아니라 **삭제**
상태였기 때문에 도커 데스크탑 Containers 탭에서 `saju_v2` 프로젝트 행이 통째로 사라져
"도커 데스크탑에도 안 보인다"로 관측됐다. CLI와 GUI는 같은 엔진을 본다
(`docker info` → `Name=docker-desktop`, WSL 통합 프록시 소켓).

### 원인 — 데이터 손실이 아니라 기동 주체 부재

DB가 날아간 것이 아니라 **볼륨을 물어줄 컨테이너가 없었다.** 볼륨이 명명 볼륨이라
데이터는 그대로였고, v1만 살아난 이유는 restart 정책 유무 차이였다. 컨테이너가 언제·무엇에
의해 삭제됐는지는 확정하지 못했다(shell history에 docker 명령 없음, 데몬 재시작으로 이벤트
버퍼 소실). 볼륨이 남은 점에서 `down -v`·factory reset은 아니다.

### 복구

`docker compose up -d`(기존 볼륨 재사용) → `./scripts/restart_backend.sh`. 데이터 전량 보존
확인: 17테이블(llm_usage 1610 · chat_messages 1136 · conversation_threads 305 · subjects 41 ·
accounts 36 …). DB 배선 확인은 `/api/v2/report/jobs` 가 **401(인증 요구)** 인지로 한다 —
503이면 DSN 미주입이다.

### 재발 방지

v2 두 서비스에 `restart: unless-stopped` 를 넣었다. `always`가 아닌 이유: 전원 차단 전
`docker compose stop` 으로 의도적으로 내린 경우까지 되살리지 않기 위해서다(수동 중지 후에도
데몬 재시작 시점에는 복귀한다).

살아있는 컨테이너에는 `docker update --restart unless-stopped saju-v2-db saju-v2-redis` 로
적용했다. `docker compose up -d` 로 정책을 먹이면 컨테이너를 재생성하며 DB가 끊기기 때문이다.

**정책의 한계**: restart 정책은 재부팅 생존만 담보하고, 이번의 직접 원인인 컨테이너 삭제는
막지 못한다. 삭제 대비는 명명 볼륨이 담당하고 복구 절차는 `docker compose up -d` 다.

같이 고친 오기: `precompute_store.py` docstring의 `호스트 5433` → `15432`
(5433은 Windows 예약 포트 범위라 바인딩 실패해 15432를 쓴다 — compose 주석에 이유 기재됨).

---

## 2026-08-05 — 오늘의 운세 스레드 게시분을 전일 21시로 (게시 기준일 도입)

### 배경 — 요구와 실제 동작의 차이

"오늘의 운세 생성을 00시에서 전일 오후 9시로" 라는 지시를 받았다. 실제 동작은 00시 생성이
아니었다. **전일 23:50 생성 + 당일 00:05 스레드 파일 갱신**의 2단계였고, 그렇게 갈라져 있던
이유가 따로 있었다.

`write_threads_export` 는 `board.fortune_date == 오늘` 일 때만 쓴다. 2026-08-01 에 8/2 보드가
이 파일을 덮어 **토요일에 일요일 운세가 스레드에 올라간** 사고의 방어물이다. 그래서 23:50
생성 시점의 export 는 언제나 하루 일러 건너뛰어졌고, 자정을 넘긴 00:05 에 한 번 더 부르는
구조가 됐다(2026-08-02).

즉 "전일 21시에 파일을 교체" 하려면 이 가드를 정면으로 통과해야 한다.

### 범위 확정 (사용자 선택)

세 가지 안 중 **"스레드용 텍스트 파일을 우선 교체하고, 서비스 노출은 00시 유지"** 를 택했다.
노출 기준일까지 옮기는 안(21시부터 사용자가 내일 운세를 봄)은 채택하지 않았다.

### 해결 — 가드 완화가 아니라 기준일 경계 이동

`threads_publish_date(now)` 하나를 SSOT 로 두고, 가드의 비교 대상을 '오늘' 에서 '게시 기준일'
로 바꿨다.

| | 기준일 | 경계 |
|---|---|---|
| 사용자 API | KST 오늘 | 00:00 (불변) |
| 스레드 export 파일 | 게시 기준일 | 21:00 (`THREADS_PUBLISH_HOUR`) |

핵심은 **통과 날짜가 여전히 정확히 하나** 라는 것이다. 21시 이후에는 익일 보드만 통과하고
당일 보드는 막힌다 — 21~24시 사이 사용자 요청으로 당일 보드가 lazy 재생성돼도 게시분을
되돌리지 못한다. 과거·D+2 보드 차단도 그대로다. 회귀
`test_allowed_date_is_unique_at_every_hour` 가 24시각 전부에서 이 유일성을 고정한다.

### 변경

- `daily_fortune_export.py`: `THREADS_PUBLISH_HOUR=21`, `threads_publish_date()` 신설.
  가드 인자 `today` → `publish_date` 개명 — 기준이 더는 "오늘인가" 가 아니므로 이름을
  거짓말로 두지 않는다.
- `main.py` `daily_fortune_pregen_schedule`: 생성 23:50 → 21:00(상수 공유). 00:05 는
  **멱등 재확인** 으로 역할 변경(21시 쓰기가 실패했고 재기동도 없었던 경우의 안전망).
- `main.py` 루프: 21시 `_ensure` 직후 `_export` 추가. 기동 즉시 보충은 당일(API 노출용)과
  게시 기준일(21시 이후 기동이면 익일)을 **모두** 채운다 — 안 그러면 21시 이후 재기동 시
  파일이 당일자에 멈춘다.
- `_export` 에서 `today=target` 인자 제거. 보드도 target 이라 비교가 언제나 참이 되어
  **가드를 우회하고 있었다.** 이제 시계가 기준일을 정해 가드가 실제로 동작한다.

### 검증

`./scripts/gates.sh` 4종. 신규 회귀: 21시 경계 정확성(20:59 당일 / 21:00 익일), 21시 이후
익일 보드 통과, 21시 이후 당일 보드의 파일 되돌림 차단, 시각별 통과 날짜 유일성,
생성 시각이 `THREADS_PUBLISH_HOUR` 상수를 따르는지.

### 운영 주의

21~24시 사이 `오늘의운세.txt` 는 **내일 것**이다. 구분은 파일 최상단 날짜 헤더
(`[오늘의 운세 — 2026-08-14 목요일]`)로 한다. 21시 이후 재기동 시 교정 배치가 당일·익일
2회 돌 수 있으나 `polish_status == RAW` 일 때만 수행되어 중복 과금은 없다.

---

## 2026-08-06 — 오늘의 운세 재생성 낭비 차단 (같은 날짜를 두 번 만들지 않는다)

### 관측 — 교정 호출의 40%가 재생성

"오늘의 운세 생성 일정이 예산을 지속적으로 낭비한다"는 지적. `llm_usage` 전수 집계:

| 총호출 | 필요호출(고유 날짜) | 낭비 | 총비용 |
|---|---|---|---|
| 25 | 15 | **10 (40%)** | $0.44 (낭비 $0.18) |

KST 로 환산하니 패턴이 한 가지로 모였다. 중복이 난 날은 07-23(4회) 07-24 07-25 07-26
07-28(3회) 07-29 08-04 이고, **호출 시각이 매일 06시 전후에 반복**된다(07-24 06:33 ·
07-25 06:58 · 07-27 06:16 · 07-28 06:14 · 07-30 06:21 · 08-04 06:17 · 08-05 06:15).
사용자 트래픽이 아니라 **서버·컨테이너 기동의 흔적**이다.

> 최초 집계에서 `created_at`(timestamptz, `+00` 표기)을 KST 로 읽어 "전일 21시 정상
> 선생성"으로 잘못 판단했다. UTC→KST 환산 후에야 당일 재생성 패턴이 드러났다.

### 원인 — 판정 로직이 아니라 판정 근거의 수명

멱등 자체는 이미 있었다. `get_board` 는 캐시 히트면 재생성하지 않고, `polish_board` 는
`polish_status == RAW` 일 때만 LLM 을 부른다. 문제는 **그 판정의 유일한 근거가 Redis 인데
볼륨이 없었다는 것**이다(compose 주석: "영속 볼륨 불필요(휘발성 콘텐츠)"). 컨테이너
recreate 마다 "이미 만들었다"는 기록이 사라져, 같은 날짜를 다시 만들고 다시 교정했다.

2026-08-05 항목의 "`polish_status == RAW` 일 때만 수행되어 중복 과금은 없다"는 서술은
**캐시가 살아 있는 동안에만** 참이었다. 이 항목이 그 전제를 보완한다.

### 변경

- `docker-compose.yml`: redis 에 명명 볼륨 `saju-v2-redisdata:/data` + `--appendonly yes`.
  내용을 영속하려는 것이 아니라 **기록을 재기동 너머로 남기기** 위해서다. TTL(운세일 익일
  03:00)은 그대로라 과거 본문은 여전히 남지 않는다 — 휘발성 원칙(docs/17 §0-2) 유지.
  - **볼륨만으로는 부족했다.** 기본 RDB 저장 조건이 `3600 1`(1시간에 1건)이라, 21시 생성
    직후 1시간 안에 recreate·재부팅이 나면 기록이 디스크에 닿지 못한 채 사라진다. 막으려는
    창이 정확히 거기라 AOF(everysec)로 노출을 1초로 줄였다. 하루 40KB 한 건이라 비용은
    무시할 수준이다.
  - 적용 순서 주의: 데이터가 있는 서버는 **런타임에 먼저** 켜야 한다
    (`redis-cli CONFIG SET appendonly yes` → 메모리에서 AOF 재작성). 플래그만 넣고 재기동하면
    빈 AOF 를 로드해 기존 보드가 사라진다. 이번 적용은 이 순서를 지켜 8/6 보드를 보존했다
    (기존 키를 TTL 째로 옮겨 담아 **재교정 0회**로 마이그레이션).
  - 검증: `docker compose up -d --force-recreate redis` 후 키 존속 + TTL 정상 감소
    (62899 → 62454). 종전 낭비를 만들던 바로 그 시나리오다.
- `main.py` 기동 보충: 대상을 **게시 기준일 하나**로 축소. 종전에는 당일과 익일을 모두
  불러 21시 이후 재기동이 교정을 최대 2회 냈는데, 당일 몫은 이미 전날 21시에 만들어
  스레드로 내보낸 것의 재생성이고 남은 수명도 3시간 미만이었다.
  21시 전 기동에서는 게시 기준일 = 오늘이라 종전과 동작이 같다.

당일 보드가 캐시에 없어도 기동 시점에 채우지 않는다. 라우터 lazy 경로가 엔진 생성으로
즉시 응답하고, 교정이 정말 필요하면 `maybe_schedule_polish` 가 **요청이 있을 때만** 한 번
낸다. 아무도 접속하지 않는 시간대의 선제 교정은 순수 낭비다.

`오늘의운세.txt` 헤더를 읽어 재생성 여부를 판정하는 안은 채택하지 않았다(사용자 확정 —
"뭘 기준으로 판정하라가 아니라 이미 있으면 다시 만들지 말라"). 판정 기준이 둘이 되면
서로 어긋날 여지가 생기고, 캐시가 이미 존재 기록이다.

### 테스트 — 원칙 1(개발 테스트에서 LLM 미사용)

`test_daily_fortune_no_regeneration.py` 신설(11건). 실호출 없이 `generate_reading` 을
**호출 횟수만 세는 스텁**으로 갈음한다(횟수가 곧 검증 대상이라 응답 내용이 필요 없다).

| 고정한 불변식 | 방법 |
|---|---|
| 같은 날짜 2회 준비 → LLM 1회 | 스텁 호출 리스트 길이 |
| FAILED 보드 자동 재시도 없음 | 빈 응답으로 FAILED 유도 후 재준비 |
| 캐시 상실만이 재호출을 연다 | 새 캐시 주입 시 2회 — 절약이 볼륨에 달렸음을 명시 |
| 기동 보충 대상은 언제나 1개 | 6개 시각 파라미터 |
| 엔진 출력 결정론 | `_generate` 2회 JSON 동일 — 재생성 무가치의 근거 |

기존 차단선은 그대로 유효함을 확인했다: conftest `_block_live_llm` 이 세션 스코프 autouse
로 `_PROVIDERS` 항목(실제 네트워크 경계)을 막고, 백그라운드 교정 예약과 export 경로도
격리돼 있다.

### 남은 한계

볼륨까지 지우면(`down -v`·새 머신) 기록이 없으므로 재생성이 일어난다. 이는 정말로 없는
것이라 옳은 동작이다. 그 경우까지 막으려면 교정 완료 보드의 내구 스냅샷이 필요한데,
휘발성 불변식과 충돌하므로 도입하지 않았다.

---

## 2026-08-06 — 답변이 두 번 끝나는 문제 + 신살 표기 훼손 (데굴님 지적)

### A. 마무리 중복 — 지시문 충돌

관측: 답변이 제안형 질문으로 한 번 맺고, 요약 한 줄 뒤 또 다른 질문으로 다시 맺는다.

원인은 모델이 아니라 **말미를 요구하는 지시가 여러 개 동시에 붙는 것**이었다. `dry_run`
프롬프트를 직접 뽑아 확인했다(LLM 미호출).

| 지시 | 위치 | 조건 |
|---|---|---|
| "답변 끝에 핵심을 한두 문장으로 정리하고 … 질문 1개" | `llm_client._SYSTEM_PROMPT` 8 | **무조건** |
| "**답 말미에** 방향 제안 1~2문장" | `DIRECTION_SUGGESTION_INSTRUCTION` | 제안 성립 시 |
| "그 날 실행할 행동 지침을 정리하라" | `_SINGLE_DAY_FOCUS_DIRECTIVE` | 특정일 질문 |
| "답변 끝에 '어느 해를 …' 물어" | `_YEAR_DIGEST_DIRECTIVE` | 막연한 시점 |
| "끝에 특정 달의 상세 … 물어라" | `_MONTH_PICK_DIRECTIVE` | '어느 달' 질문 |

`[제안 방향]` 블록은 report 전용이 아니라 `context_reducer.py:2650` 을 통해 **대화 프롬프트
에도** 들어간다(초기 판단 정정).

실측(`chat_messages`): 본문형 assistant 507건 중 **40건(7.9%)** 이 말미 5문장에 물음표 문장을
2개 이상 가졌고, **52건(10.3%)** 에 '핵심 정리/요약' 문구가 있었다. 답변 본문을 코드가 덧붙이는
곳은 없다 — 전부 LLM 출력이고 원인은 지시문이다.

**변경 — 말미 소유권을 한 곳으로.** 후처리로 문장을 잘라내지 않는다(본문을 깎을 위험).

- `_SYSTEM_PROMPT` 8: "마무리는 이 한 번뿐이다 … 물음표로 끝나는 문장은 답 전체에서 이
  마지막 질문 하나뿐" 을 명시. 다른 지시가 정한 소재를 이 질문 하나에 담게 한다.
- `DIRECTION_SUGGESTION_INSTRUCTION`: "답 말미에" → "본문 흐름 안에" + 별도 마무리 문단·
  질문 금지. (대화·리포트 공용 문구지만 `_REPORT_SYSTEM_PROMPT` 에는 마무리 규칙이 없어
  리포트 동작에는 영향이 없다.)
- `_YEAR_DIGEST_DIRECTIVE`·`_MONTH_PICK_DIRECTIVE`: "물어라" → "마무리 질문을 따로 더
  만들지 말고 그 하나의 **소재**로 삼아라".

검증: 4개 질문 유형(오늘 운세·내일 운세·올해·언제쯤) 모두 말미 소유 지시 1건. 실제 출력
빈도는 라이브 관측이 필요하다 — 여기서 고정한 것은 지시문 계약이다.

### B. 격격살 — 데이터가 아니라 LLM 훼손

`격격살` 은 리포지토리 어디에도 없다. 엔진은 `격각살`(隔角殺)만 방출하고, 집계기가 쓰는
이름 중 사전(`sinsal_text.json`)에 없는 것도 0건이다. 즉 사전·엔진은 정상이다.

이력 570건 전수 검사에서 사전에 없는 신살형 토큰 17종이 나왔으나, **존재하지 않는 용어는
'격격살' 1건(msg 1678)뿐**이었다. 나머지는 통용 변형(현침살 5·백호살 1·연살 1)이거나 정상
명리어(형살·흉살·관살·식신제살·상관제살·재생살)·일반어(몸살·햇살·살림살·보살)였다.

**변경 — 없는 말만 고친다.**

- `chart_interpretation.canonical_sinsal_names()` 신설 — 이름이 정의된 곳에 SSOT 를 둔다.
- `llm_client._normalize_sinsal_terms()` 를 `_sanitize_output`(채팅·리포트·양 공급자 공용
  단일 지점)에 연결. `_SINSAL_TYPO_FIX = {"격격살": "격각살"}`.
- 통용 변형은 손대지 않는다(사용자 확정) — '연살'(年殺)은 두음법칙상 정당한 독음이고
  '현침살'·'백호살'도 통용 표기다. 여기까지 고치면 교정이 아니라 개입이다.
- 새 훼손은 `sinsal_term_drift` 경고로 드러낸다. 탐지는 **같은 길이·한 글자 치환**으로
  한정했다 — 삽입·삭제까지 넓히면 '재생살'↔'재살', '현침살'↔'현침' 처럼 정상 용어와 통용
  변형이 매번 걸려 로그가 소음이 된다.

검증: msg 1678 원문에 적용해 `격격살` → `격각살` 복원 확인.

### 테스트

`test_answer_closing_and_terms.py` 20건. 실호출 없음(마무리는 `dry_run` 프롬프트 문자열로,
표기는 순수 함수로 판정 — 원칙 1).

초판 결함 2건을 스스로 잡았다: ①탐지 정규식 `[가-힣]{3,5}(?:살|귀인)` 이 접미사 앞 3자를
요구해 **'격격살'(3자)을 아예 못 잡았다** ②`system_prompt` 이 비었을 때 실제로는
`_SYSTEM_PROMPT` 이 전송되는데 테스트가 그 폴백을 반영하지 않아 0건으로 읽었다.

---

## 2026-08-06 — 내일 운세를 '오늘'로 부르던 결함 (기간 블록 시점 지칭)

### 관측

'내일 운세를 알려줘' 답변이 "오늘 하루는…" 으로 시작한다(데굴님 지적, 별건 진행).

실측(user→assistant 570쌍):

| 질문 | 답변에 반대 시점어 등장 |
|---|---|
| '내일' 17건 | **5건(29%)** 이 '오늘' — 예: "오늘 하루는 겉으로 드러나는 상황은…" |
| '다음달' 2건 | 1건이 '이번 달' |
| '내년' 9건 | 5건이 '올해' — 다만 "올해나 내년의 횡재수는?" 처럼 사용자가 둘 다 물은 경우가 섞여 과대평가다 |

가장 깨끗한 지표는 '내일' 29% 다.

### 원인 — 프롬프트가 거짓 라벨을 말하고 있었다

`_PERIOD_FORTUNE_HEADER` 의 헤더 라벨이 대상 기간과 무관하게 고정이었다.

    내일 운세    [오늘의 운세 — 2026-08-07 …]
    모레 운세    [오늘의 운세 — 2026-08-08 …]
    다음 달      [이번 달 총운 — 2026-09 …]
    내년         [올해 총운 — 2027 …]

헤더 뒤에 정확한 `period_label` 이 붙고 `[기준 시점]` 도 오늘을 따로 알려주지만, 상충하는
두 신호 중 LLM 은 앞의 말을 따라갔다. `dry_run` 프롬프트로 확인했다.

곁가지 둘도 같이 나왔다.

- `_SINGLE_DAY_FOCUS_DIRECTIVE` 가 `[해당 일 일운]` 블록을 가리켰는데 **그런 헤더는 어디에도
  없었다**(실제는 `[오늘의 운세]`). 끊어진 참조라 근거 지시가 조용히 무력했다.
- `_DAILY_INSTRUCTION` 의 "'오늘 일어난다'고 단정하지 말고" 가 다른 날 질문에서도 '오늘'을
  한 번 더 각인시켰다.

### 변경

- `_PERIOD_FORTUNE_HEADER`: 상대 표현 제거 → `해당 일 운세` · `해당 월 총운` · `해당 연 총운`.
  헤더에 이미 정확한 기간이 붙으므로 거짓 라벨만 걷어내면 모순이 사라진다.
- 세 기간 지시문: 블록명 참조 갱신 + "그 날/달/해를 지칭할 때는 [기준 시점]과 대조해 정확히
  부를 것 — 오늘/이번 달/올해가 아닌 것을 그렇게 쓰지 말 것" 추가. `'오늘 일어난다'` →
  `'그날 일어난다'`.
- `_SINGLE_DAY_FOCUS_DIRECTIVE`: 블록명을 실제 헤더로 맞추고, 기준 시점 대비 관계를
  `_relative_day_label()` 로 계산해 함께 넘긴다("2026-08-07 = 기준 시점 대비 내일").
- 어휘는 `time_parser.DAY_WORD_OFFSETS`(기존 `_DAY_WORDS` 공개 별칭)를 역인덱스로 재사용한다
  — 파싱과 서술이 다른 말을 쓰면 사용자가 '모레'라 물었는데 답은 '2일 뒤'라고 부른다.

헤더에서 상대 표현을 **계산**하는 안(내일이면 "내일의 운세")은 택하지 않았다. 기간 블록은
`period_label` 만 갖고 기준일을 모르는데, 그걸 넣자고 직렬화 계층에 기준일을 배선하면 같은
정보가 두 곳에서 상대 표현을 만들게 된다. 지칭은 `[기준 시점]`·특정일 지시문 한 곳에서만
정하게 둔다.

### 검증

`test_period_reference_labels.py` 23건(LLM 미호출). 5개 질문 유형에서 헤더에 고정 상대
표현이 없고, 특정일 지시문이 오늘/내일/모레를 정확히 싣고, 지시문의 블록명이 실재하는지
고정한다. 실제 출력 문장의 지칭 개선은 라이브 관측 몫이다.

### 함께 고친 것 — 앞선 회귀의 거짓 통과

같은 실행에서 `test_answer_closing_and_terms.py::test_unknown_drift_is_logged_not_silently_kept`
가 **단독 통과·전체 스위트 실패**로 갈렸다. 원인은 `main.py` 가 앱 네임스페이스 로거에
`propagate = False` 를 걸기 때문이다(서드파티 INFO 소음 차단). caplog 는 루트에 붙으므로,
`saju_api.main` 을 import 한 테스트가 하나라도 앞서 돌면 경고가 실제로 났는데도
`caplog.records` 가 빈다.

**부정 검사가 더 위험했다** — "일반어로는 로그가 울리지 않는다" 가 조용히 거짓 통과하고
있었다. 전용 fixture 로 해당 로거에 caplog 핸들러를 직접 붙이고, 포착 배선 자체를 검증하는
테스트(`test_the_drift_logger_is_actually_captured`)를 앞세워 같은 함정을 막았다.

---

## 2026-08-06 — 21시 게시가 50분 일찍 발화해 실패 (벽시계 보정 vs monotonic sleep)

### 관측

21:11 에 상태를 확인하니 8/7 보드는 생성·교정(POLISHED)까지 끝났는데 **스레드 파일이
아직 8/6** 이었다. 로그에 실패가 그대로 남아 있었다.

    오늘의 운세 export 건너뜀 — 게시 기준일(2026-08-06) 보드가 아니다: 2026-08-07  ×3
    스레드 export 건너뜀 date=2026-08-07

교정 호출 시각은 21:00 이 아니라 **20:13:57** 이었다. 그 시각엔 게시 기준일이 아직 당일
이라 가드가 익일 보드를 막았다. **가드는 설계대로 동작했다 — 어긋난 것은 발화 시각이다.**

### 원인 — 벽시계는 뒤로 밀리는데 sleep 은 monotonic

    로그 기록 기동 시각 : 08:56:15   (그 시점의 벽시계)
    ps 계산 시작 시각   : 08:05:47   (monotonic 역산)
                          ─────────
    차이                : 50분 28초

    monotonic 경과 47,275초  vs  벽시계 경과 44,247초 (08:56:15 → 21:13:49)

기동 시 "21:00 까지 43,425초" 를 벽시계로 한 번 계산하고 `asyncio.sleep`(monotonic)으로
그대로 잤다. 자는 동안 벽시계가 50분 28초 뒤로 밀리면서, monotonic 기준 21:00 이 실제
벽시계로는 20:09 가 됐다. 교정에 4분쯤 걸려 20:13 기록과 맞는다.

확인 시점의 시계는 정합했다(부팅+uptime vs wall = −0.4초). WSL2 가 호스트와 주기적으로
시간을 맞추며 생긴 보정으로 보이고, **이 계열의 점프는 재발한다.**

### 변경 — 벽시계 기준으로 기다린다

`main.sleep_until(target, step=60)` 신설. 남은 시간을 한 번 계산해 자지 않고 `step` 이내로
나눠 자며 **매번 벽시계를 다시 읽는다.** 어긋남이 한 조각(60초) 이내로 제한되고, 벽시계가
앞으로 뛰어 목표를 지났으면 즉시 반환한다. 하루 한 번 태스크에 깨어남 약 780회 — 무시할
수준이다. 루프의 두 대기(21:00 생성, 00:05 재확인) 모두 이걸 쓴다.

### 즉시 조치

백엔드 재기동(21:16). 기동 보충 대상이 게시 기준일(21시 이후 → 8/7)이고 그 보드가 이미
캐시에 POLISHED 로 있어 **LLM 호출 0회**로 `_export` 만 통과했다. 파일이 8/7 로 교체됐고
API 노출은 8/6 그대로다(자정 기준 유지).

### 테스트 — 초판이 결함을 못 잡았다

`_FakeClock` 으로 "자는 도중 벽시계 후진" 을 주입한다. **초판은 점프를 sleep 횟수로 걸어
무의미했다** — 한 번만 자는 옛 구현에는 점프가 닿지 않아 결함이 있는 코드도 통과했다.
경과 시간(monotonic) 기준으로 바꾸고, 검사 도구 자체를 검증하는 테스트를 넣었다.

| 테스트 | 고정하는 것 |
|---|---|
| `test_the_fake_clock_actually_reproduces_the_defect` | 옛 구현을 같은 시계에 돌려 20:09:32 에 깨어남을 확인 — 이게 실패하면 아래가 무의미 |
| `test_backward_clock_jump_does_not_fire_early` | 새 구현은 벽시계 21:00 전에 깨지 않음 |
...
## 2026-09-01 — 페르소나 공통 문장 결 문단: AI 문체 패턴 교정 ✅

프롬프트 글쓰기 자료(주어·목적어 생략 / 어순 변주 / 문장 길이 변주 / 상투구 회피, ChatGPT 보완
3건 반영)를 바탕으로 **모든 페르소나 조합**의 어투를 교정하는 고정 문단을 페르소나 블록에 추가
(사용자 승인 2026-09-01).

- **배경**: 페르소나 5축(성별·연령·말투·난이도·호칭)은 종결어미·호칭만 통제하고, 문장 결
  (주어를 꼬박 쓰는 완결성, 정석 어순 나열, 일정한 문장 길이, "결론적으로/핵심은" 류 상투구)에
  대한 지침이 프롬프트 어디에도 없었다 → AI가 정리한 글처럼 읽히는 원인.
- **구현**: `saju_engines/persona.py` `_PERSONA_TEMPLATE` 끝에 슬롯 없는 고정 문단
  `[문장 결 — 모든 페르소나 공통]` 추가 — 슬롯 치환부 뒤에 붙으므로 "템플릿 치환으로만 생성"
  (절대 원칙 12) 유지, 채팅(`chat_service`)·리포트(`report_service`) 두 주입점에 자동 적용.
  긍정형 프레임·"문맥에 맞을 때만"으로 묶고 말줄임표 강제·강한 명령어는 배제. 마지막 줄로
  변주를 종결어미·존대·호칭 규칙 안에 잠가 리포트 준수 검사(≥93%·혼용 0)와 충돌 방지.
  `_SYSTEM_PROMPT`(캐시 고정 prefix)·점수·판정 경로 무접촉. 프롬프트 +약 560자/호출.
- **문서**: docs/11 5-3 템플릿 전문을 코드 현행으로 동기화(호칭 "최대 2회" 06-14 · 말버릇
  "현재 말투로 변환" 06-23 개정분이 문서에 미반영이던 것도 함께 정정). 5-4 준수 검사
  95%↔코드 93% 불일치는 별건으로 미조치.
- **테스트**: `test_persona_block_has_shared_prose_guide` — 해요체/반말체/합쇼체 3조합에서
  고정 문단 동일·슬롯부 상이·슬롯 누출 없음·순서(금지 고지→공통 문단) 검증. 기존 페르소나·골든
  스타일·프롬프트 품질 테스트 66건 비회귀.
- **검증**: VALID_SUITE_PASS · All checks passed · production mypy gate clean ·
  maintained scripts mypy gate clean.

## 2026-09-01 — 생활형 횡재 오탐 수정: 주식 장기 투자 질문이 '오늘 복권' 답으로 흐르던 결함 ✅

실사용: "주식의 장기 투자가 실제 내 이익으로 돌아올까?" → 답 전체가 "오늘 복권 한 장·당첨
기대치·구매 장소·직관이 느껴진 번호"로 흘렀다(사용자 보고). 파서는 정상(wealth/wealth_change/
timeless)이었고, 원인은 `chat_service._is_lifestyle_windfall`이 재물 도메인 + 질문에 "주식"
문자열만 있으면 참이 되어 `_LIFESTYLE_WINDFALL_DIRECTIVE`("이 질문은 로또·연금복권·소액 주식
같은 생활형 횡재다 … 유리한 시기·태도")가 붙은 것. 지시문 프레임이 답을 통째로 끌고 갔다.

- **판정 2단화**: 강한 키(로또·복권·연금복권·경마·토토)는 단독 횡재. 약한 키(주식·코인·
  비트코인·펀드·청약)는 투자 표지(`_INVESTMENT_MARKERS`: 장기/중장기/투자/수익률/수익/이익/
  손실/원금/자산/포트폴리오/적립/배당/매도 시점/손절/익절/물타기)가 없을 때만 횡재.
- **`_INVESTMENT_FLOW_DIRECTIVE` 신설**: 약한 키 + 투자 표지 → 주제를 재성 세력·용신 여부·
  축적/소모 시기·축적형/회전형으로 고정하고 복권·당첨·오늘 하루·구매 장소 프레임 배제.
  절대원칙 8 하드 가드 ①종목 픽 ②결과 단정 ③과몰입 권유는 동일 유지. 횡재 지시문과
  상호 배타(elif). 시간 지평은 [답변 지평] 디렉티브 단일 소유 — 여기서 재정의하지 않음.
- **파서 무접촉**: 종목 픽 OUT_OF_SCOPE 거부 그대로. 청약은 추첨형 선발 코어와 별개로
  어휘 판정만.
- **발견(별건, 미조치)**: 코인·펀드·청약·비트코인은 `Domain.WEALTH` 어휘에 없어 "코인 자산
  원금 회복될까"·"펀드 수익률 괜찮을까"·"비트코인 지금 사도 될까"가 `general`로 떨어진다
  (기존 갭 — 횡재 지시문도 투자 지시문도 안 붙음). 재물 어휘 등재는 도메인 라우팅 영향이
  있어 승인 후 별도 작업. 상품 어휘 없는 "장기 투자 수익이 날까"는 설계상 일반 재물 풀이.
- **문서**: CLAUDE.md 절대원칙 8에 투자 표지 예외 추가.
- **테스트**: `test_lifestyle_windfall_policy.py` +5(투자 질문 비횡재·흐름 질문 횡재 유지·
...
## 2026-09-10 — 오늘의 운세 결 층 2차: 내부 전수 감수 + 10십성 세분 + 연애 라인 결 적용 ✅

데굴님 지시 "남은 것들 마무리 진행" + "별도 승인 건도 진행". 어제 작업분(§22-7·§23)은 게이트 4종
재확인 후 먼저 커밋(adde952)하고, 2차 개정을 같은 dict.v1.13 에 in-place 로 담았다(9/11 보드까지
v1.12 라 라이브 전 — 버전 범프 없음, MODEL_V2_VERSION 도 불변).

- **감수 A(5군 행동 640)**: 교체 42(high 6·medium 36). 공돈에 지출 권유·득템에 구매 보류·의견 수용에
  윗사람 영합 같은 사건 밖/방향 반전, 서술문 16, 기호·길이 7, "무조건". 감수자 제안 중 2건은 수동
  교정(love_reunion 재성 "안부까지만", praise_recognition "확실히"→"빠짐없이").
- **감수 B(결과 42 × 사건 64 = 1,344조합)**: 모순 84. 문장 교체 16 + 사건별 채널 제외 10사건 18슬롯
  (`stage_result_exclude` — 제외 채널이면 사건 기본 결과 풀, 행동은 결 풀 유지, 3문장 유지).
- **10십성 세분(별도 승인)**: `sipseong_actions` 64×10×2 = 1,280문장. 십성 풀 우선, 없으면 5군
  폴백. 서브에이전트 4분할 집필 → 병합 검증(금지어·한자·길이·사건 내 고유) 0건 → 표본 검토 후
  2문장 수동 교정(love_reunion 비견 "공통 친구를 통해 근황" 제거, body_light 편관 "힘든 일 몰아서").
- **연애 라인 결 적용(별도 승인)**: `_love_line(tone=)`. 결 키 없는 사전은 바이트 불변.
- **카탈로그 감수 5건은 미적용**(채점 규칙 — 사문 감점 2·게이트-evidence 불일치 2·opinion_accepted
  상관+정관 동시 가점). §22-3 개정·재측정이 따르므로 사용자 판단 대기(docs/17 §23-5).
- **실측(9/12 보드)**: 일간 그룹 헤드라인 고유 6/6(10그룹 전부, 1차는 3.04), 십성 풀 적중 60/60,
  연애 라인 3문장. 선발·점수 불변.
...
## 2026-09-10 — 오늘의 운세 3차: love_spark 삼합 evidence + 12운성 결과 풀 12스테이지 세분 ✅

데굴님 지시(남은 두 건 진행). 둘 다 dict.v1.13·model.v2.1 in-place(라이브 전).

- **love_spark**: 게이트 삼합에 evidence 없음 → 삼합 .3(육합 .5 보다 낮게 — 1:1 결속이 주축). v1 은 손으로
  쓴 three_harmony .6·half .4 가 이미 있어 무변경. 게이트 리프 회귀를 전 64종으로 확대.
- **12스테이지 결과 풀**: `stage_results_by_stage` 72문장, `DayTone.stage`, 스테이지 풀 우선 → 7채널 폴백,
  채널 제외는 속한 스테이지에도 적용. 서브에이전트 감수(실현 가능 스테이지 683쌍 × 3 = 2,049조합):
  모순 68 → 교체 23(제외 0). 발견: procrastination 은 게이트상 병·사·절 날에만 뜨는데 세 채널이 제외라
  결 풀 조합 0 — 항상 사건 기본 풀(설계대로).
- **실측(9/12)**: 일간 그룹 결과 문장 고유 5.80→6.00/6, 같은 헤드라인 공유 37쌍 동일 결과 0.
- 테스트: 스테이지 풀 커버리지·중복 금지, day_tone.stage 정의, 스테이지 우선/채널 폴백, 제외 전파,
  그룹 내 비교를 스테이지 기준으로. 문서: docs/17 §22-3 love_spark 행, §23-1/2/3/4, §23-6 이력.

## 2026-09-10 — 오늘의 운세 4차: §22-6 P1 "12운성 잔여 채널" — 채널 정의·엔진(inert) + 배선 제안 shadow ✅

데굴님 지시. 문서에 채널 정의가 없어(§22-6 은 이름만) §23-1 이 승인한 스테이지 의미를 채점 채널로
승격하는 설계를 택했다: 흔들림 unsettled(목욕 1.0) / 단장 poised(관대 1.0) / 노련 seasoned(쇠 1.0) /
구상 incubation(태 1.0·양 .8). 기존 7채널 값은 가산으로 보존 → 사전이 참조하기 전까지 선발 바이트 불변
...
## 2026-09-10 — 선별 상충·클론 감사 상설화 (권장 순서 4 — 1단계) ✅ / 클론 캡은 승인 대기

40명식 × 2026년 shadow(스크래치패드 `shadow_conflict_audit.py`·`shadow_fanout_refill.py`):
- **상충(같은 시기·같은 도메인·반대 방향) 동시 노출 = 0건** — 풀·채팅 총운 Top-5·리포트 표 12행 모두.
  클러스터 축이 반대 방향을 분리하는 건 2026-07-14 확정 설계라, 막을 대상을 '같은 시기' 로 한정했다.
  회귀 `test_selection_contradiction_guard.py`(8명식) 가 0건을 고정.
- **클론(같은 시기·다른 사건·신호 벡터 cos≥0.85)**: 채팅 총운 2.05쌍/명식, 리포트 표 9.03쌍/명식
  (표 12행 중 한 시기 최대 3.77행). daily 감사의 "한 신호가 여러 사건으로 복제" 가 리포트에 그대로.
- **캡 제안(period×지배 신호 ≤2, 재충원)**: 채팅 클론 2.05→1.23, 시기 3.48→3.77, 도메인 3.20→3.33,
  13/40 명식 변화 / 리포트 클론 9.03→5.17, 시기 최대 행 3.77→2.00, 시기 6.47→6.83, 40/40 변화.
...
## 2026-09-10 — 오늘의 운세 풀이 이모지·특수기호 금지 ✅ (데굴님 지시)

- `saju_engines/daily_text_policy.py`: `strip_symbols`(어절 끝 기호→마침표, 그 외 제거)·`sanitize_board`(기호 없으면 원본 반환).
- 세 지점 적용: 사전 `daily_phrase_templates.json` 87문장(♪86·♥1) 정리 + dict.v1.13 스냅샷 재빌드 / 퇴고 프롬프트 규칙 8
...
## 2026-09-17 — 실로그 결함 점검(로또 좋은 달 → 9월 소식 추석 전/후) ✅ (데굴님 지시)

- **관측 결함**(2턴 대화): ①추석이 시점으로 안 잡혀 월운으로 얼버무리고 질문 회피 ②후속 턴 '제안·소식'이 직전 사건(횡재)을
  승계해 주제 이탈 ③단일 달(9월) 질문에 월별 요약·유력 달 종합이 12개월(5월·12월 골자) ④월별 요약 행에 십성이 없어 LLM이
  丁(己일간 편인)을 '정인'으로 계산 ⑤직전 답변의 되물음("계약이나 이사 소식 있으신가요?")을 '말씀하신'으로 사용자 귀속 +
  프로필에 없는 '엔지니어링 업무' 날조 ⑥'9력 9월' 표기 오염.
- **명절 앵커**: time_parser `holiday_anchor`(추석·한가위 / 설날·구정 / 신정 / 크리스마스, 음력은 lunar_to_solar) →
  `parse_time_with_constraints`에서 시점에 `anchor_dates` 부착(시점 없으면 그 명절이 든 단일 달). 45일 지난 명절은 내년.
  chat `_anchor_window_day_note`: 앵커 ±7일(최대 24일) 일운 간지·십성·등급 나열 + 전/후 구간 등급 집계(엔진 집계) +
  3진 판정 지시.
- **사건 어휘**: EVENT_WORDS contract_document에 제안·제의('오퍼'는 부동산·투자 맥락 모호로 제외 정책 유지) — 후속 턴이 새 사건을 들고 오므로 직전 횡재를 승계하지 않는다.
- **단일 달 창**: chat 월 창 분기에 `len(start)==7 and end in (None, start)` → `window_months=[그 달]`(이전엔 연도 분기로 12개월).
- **십성 병기**: context_reducer 월별 요약 행 `[丁火 편인·희신 / 酉金 식신·한신]`(LuckPillar stem/branch_ten_god) + 범례에
  '십성은 표기를 그대로 쓰고 직접 계산하지 말 것'.
- **귀속 규칙**: `_THREAD_ATTRIBUTION_RULE`(후속 턴 공통) — 되물음은 사용자 사실 아님, 직업·상황 날조 금지. 현재 절기월
  안내에 달 명칭 표기 규칙('9월(丁酉월)', '○력 N월' 금지).
- 회귀: `test_realtime_log_misses_20260917.py` 7건. 게이트 4종 통과.

## 2026-09-17 — 실로그 결함 점검 2 ('그럼 내 사주에서 주의해야할 점은 뭐야?') ✅ (데굴님 지시)

- **관측**: 명식 전반 주의점 질문이 스레드에서 직전 시점(9월)·도메인(재물/직업)을 승계해 '9월 이직 제안' 답으로 이탈.
  단독으로 물어도 REMEDY(Q10)+무시점·무분야 → too_broad('범위가 넓어요') 안내. 반면 '내 사주의 단점은?'은 Q8로 잡혀 정상.
  부수: 직전 턴의 추석 앵커가 승계된 시점 창을 타고 다음 턴까지 따라와 일운 창이 새는 누수.
- **라우팅**: query_parser Q10에서 명식 범위 표지(`_CHART_SCOPE_RE`: 내/제/본인 사주·명식·원국, 사주에서/상/의) + 분야 없음이면
  Q8(CHART_ANALYSIS)로 보내고 `IntentJson.chart_caution=True`(주의·조심·약점·단점·취약·리스크). 분야가 붙은 개운 질문은 기존 경로.
- **승계 차단**: conversation 도메인 승계 2곳·시점 승계에 chart_caution 가드(career_field와 동일 원리 — 원국 축).
- **지시문**: chat `_CHART_CAUTION_DIRECTIVE`(기신·구신 작동 조건 / 과다·결핍 / 불리 구조 패턴 / 원국 배경 — 시점 사건 금지,
...
## 2026-09-18 — 전문가 참고 기준(관계 구조·십성 작용·신살 층위) 반영: 사전 정비 + 4단 서술 + 판정 소비 보강 ✅ (데굴님 승인)

- **현황 조사 결론**: 참고 기준의 용어 대부분은 이미 엔진이 판정·소비 중(합반·합거·탐합망극·합화·암합, 충형파해·삼형
  2글자·인사신 일지 조건, 상관견관·관살혼잡·재다신약 등 용신/격국/시험 fav 패턴, 복음·반음, 신살 9종 보조 modifier).
  누락은 ① 동의어 별칭 6건, ② 쟁합 태그의 '삼각 구도' 기본 문구, ③ 암합 표현 가드, ④ 서술 형식(작용→영역→양상→성립 조건),
  ⑤ 희용신 합반(harm) 미소비, ⑥ 합처봉충·쟁합 십성·세운병림 이름 없음. 원진은 관계 hit에 없고 신살 채널 1회만 계산(중복 없음).
  interactions의 '반음' 정의는 참고 기준의 천극지충(천간극+지지충)과 같다.
- **사전 정비(스냅샷 v1.2.0 → v1.3.0)**: 별칭 겁재탈재·재자살·신약살왕·편인탈식·식상태과·인성태과(별칭 보유 항목 19→22),
  쟁합·투합 llm_tag "기회·관심·이익 배분의 경쟁, 관계의 전일성 부족(삼각관계·몫 절반 단정 금지)". 탐재괴인↔재극인 통합은
  전문가 확인 대기라 유지. 암합 줄 라벨에 "외도·배신의 직접 근거 아님" 가드(항상).
- **RELATION_TERMS_V2(플래그 `SAJU_RELATION_TERMS_V2_ENABLED`, 기본 OFF, .env.beta ON — 서술·표기 전용, 점수 불변)**:
  - 구조 패턴 블록을 `이름: [작용] llm_tag [영역·후보] domain_hints [양상] polarity_mode [성립 조건] evidence` 4단으로 렌더
    (`context_reducer._pattern_four_stage_line`) — 새 명리 규칙 없이 사전 필드 재배열.
  - `hap_modes.StemHapResolution.contested_stem` 추가 → `hap_lines` 쟁합 표기에 십성군(관성/재성/인성/식상/비겁 쟁합 — 십성 글자) 병기.
  - `hap_lines.luck_hap_mode_lines` 육합 줄에 합처봉충(원국 지지가 합 글자를 충) 표기.
  - `context_reducer._sewoon_line` 세운병림(대운=세운 간지) 표지 — "그 자체는 흉이 아님".
- **HAP_MITIGATION 확장(판정 층, 같은 플래그)**: `hap_mitigation`에 stem_harmed/branch_harmed — 운 글자가 원국 용·희신을 합거로
  묶으면 희용신 손상. 합 글자 중 공망지가 있으면 合則不能空이 우선이라 제외(데굴 巳申合). 이벤트 fav_adj −0.3(`HAP_HARM_PENALTY`,
  reason `制_합거_길신손상`), 월 등급 −0.1(`HAP_HARM_MODIFIER`) + 요약 문구. 데굴 12개월 창에는 해당 달 없음(판정기 단위 테스트로 검증).
- 회귀: `tests/unit/test_relation_terms_v2.py`(4단·세운병림·쟁합 십성·합처봉충·harm 판정), f6 별칭 수 22로 갱신.
- **보류(승인 필요, 문서에 없는 규칙)**: 運破格(운이 격국 상신 손상)·기신 방향 성국 배경 감점. 신살 modifier 가중치(백호 0.15 등)는
  전문가 감수 항목.

## 2026-09-18 — 4번 運破格·기신 성국 배경 감점 + 5번 신살 보조 약화 ✅ (데굴님 승인)

- **運破格** (`saju_manse_analysis/structure/luck_geok_break.py`, 순수 판정): 격신 십성별 파격·구응 표(정관격: 상관→인성/재성,
  편관→식신/인성 · 편관격: 정관→식신/인성, 재성→식신/인성 · 재격: 비겁→관성/식상 · 인격: 재성→비겁/관성 · 식신격: 편인→재성 ·
  상관격: 정관→인성/재성; 건록·양인·특수격은 판정 안 함). 운 천간·지지(본기) 십성이 파격 십성이고 원국(일간 제외 천간·지지 본기)에
  구응이 없으면 `GEOK_BREAK_PENALTY` 0.2 fav 감점(`格_운파격_{구조}`), 구응이 있으면 감점 없이 `格_운파격경향_{구조}` 코드만.
  데굴 정재격: 2026-10 戊戌 비겁 유입 → 원국 庚 상관 구응 → '경향'만.
- **기신 성국**: 운 지지 관계가 삼합완성·반합성립·방합완성이고 국 오행이 기·구신이면 `GISIN_LOCAL_PENALTY` 0.2 fav 감점
  (`局_기신성국_{오행}`). 월 등급은 기존 변환 오행 보정(−0.1)이 이미 있어 요약 문구만 추가. 데굴 2026-12 庚子(申子 반합 水).
- 둘 다 `event_engine_v2._apply_structure_background`(HAP_MITIGATION 플래그 공유), favorability만 — 점수·활성·순위 ON/OFF 동일.
  한 시점에 항목당 최대 1회 적용(충·용신 손상·파격 중복 계산 방지). 근거 코드 한글 라벨 3종 추가.
- **5번 신살 약화**: `sinsal_modifier_config` 흉살 shadow 계수 약 절반(백호 0.15→0.08, 그 외 0.12→0.07, 원진 0.06, 망신 0.05,
  favorability −0.02~−0.03, 캡 favorability 0.08·risk 0.12). **이 채널은 SINSAL_NUMERIC_SHADOW_ENABLED=False라 운영 점수에
  미반영**이며, 운영에 실제로 나가는 것은 EFFECT_TAGS 문구다 → 백호·귀문·원진·고신·과숙·망신·양인·괴강·겁살 문구를 참고 기준대로
  '상징 + 확정 아님' 형으로 완화.
- 회귀: `tests/unit/test_structure_background.py`(규칙표·fav 감점·월 요약 표기). 시험 대상 후보는 exam 보정기가 fav_adj ±0.5로
  클램프하므로 정확값 검증은 재물 후보로.

## 2026-09-18 — 전문가 보완 자료(작용 확장·사건 목록 부정/긍정) 반영: B1 사전·B2 계약·B3 오늘의운세·B6 감사·A2 판정·A1 신살 ✅ (데굴님 승인)

- **순서**: B1→B2→B3→B6→A2→A1(데굴님 확정). 개두·절각 원국 적용은 고전 근거(옥조정진경 사례)만 있고 실증 근거가 없어
  판정은 운 기둥에만, 원국 기둥은 표지로(절충). 신살 7종은 통설표(성별 부재라 연간 음양으로 陽/陰年 대신).
- **B1 사전(검증 스키마 등록, 런타임 원본 로드)**: `event_forms.json` 21종 전량(v1.1.0, 반대 발현 포함, 기존 4종 prob 합 정규화);
  `event_process_types.json` 공통 사건 유형 29종(부정 15·긍정 14 + 결정론 match 규칙); `life_event_lexicon.json` 16 영역 부정·긍정
  사건, 체감→관찰 가능 사건 번역 표 11행, 발생→진행→결과→후속(부정 6)/기회→진행→성취→유지(긍정 6) 사례, 개선 구조→긍정 범주 8,
  분리 원칙 5. `dictionaries.py` 스키마 3종·lint(키·prob 합·id 중복).
- **B2 계약(플래그 `SAJU_EVENT_LEXICON_ENABLED`)**: `event_lexicon.py` — 후보별 '사건 유형(엔진 분류)' 줄(`LlmEventCandidate.process_types`,
  최대 3, 사전 순서)·범례·사건 서술 계약 지시문(채팅 trailing·리포트 prefix, 사전 SSOT). 경쟁≠탈락≠손실, 기회≠성취≠유지, 구체 사건 확인 전
  선택 금지, 12운성·묘고 가드.
- **B3 오늘의 운세**: polish 프롬프트 9) 사건 어휘 규칙(체감→눈에 보이는 일, 좋은 일=풀림·회복 포함, 나쁜 일=종류까지, 병·죽음·이별 단정
  금지) — PROMPT_REVISION 2026-09-18(캐시 namespace 불변). 카탈로그 태깅은 `life_event_lexicon.daily_domain_to_area`로 준비만.
- **B6 감사**: `month_coverage_audit` 검사 ③ 손실 확정어(빼앗기·잃·손해·날리·탕진…) 문장의 달 후보 유형에 '손실·손상'이 없으면 교정
  문단(LLM 재호출 없음). chat_service가 후보 유형을 달별로 넘긴다.
- **A2 판정(플래그 `SAJU_STRUCTURE_BACKGROUND_ENABLED`)**: `structure/luck_structure_flags.py` — 개두·절각(운 기둥, 억제되는 쪽 역할로
  길흉 방향), 원국 기둥 표지, 충근(본기·유일 뿌리·역할), 통관 부재(운↔일간 극에 통관 오행 표면 부재), 구응 손상(運破格 구응 글자가 운
  충·합거로 전부 사망 시 '경향'→'파격'), 특수격 역행(전왕 극·종격 일간 방조). 이벤트 fav: 충근 ±0.2/0.1, 개두절각·통관 ±0.1/0.05,
  구응 손상·특수격 역행 −0.2(코드 10종 한글 라벨). 월 등급: 억제 쪽 ×0.85 + 요약 표기. 화기격 파괴는 제외(化神 경로 별도).
- **A1 신살 7종(통설표)**: 음양차착(일주 12)·고란살(일주 5)·탕화살(일지 丑寅午)·상문(연지+2)·조객(연지−2)·구교살(연지 ±3, 연간 음양)·
  원진(元辰, 연지+7/+5 연간 음양). 카탈로그 메타 caution, 표시·해석 보조. EFFECT_TAGS 미등록은 polarity 기본 문구로 폴백.
- 회귀: `test_event_lexicon.py`(사전·분류·지시문·손실 감사), `test_luck_structure_flags.py`(A2 5종·A1 표 대조). validate_dictionaries 96개 통과.
- **남은 것**: 사전 3종 reviewed:false(전문가 감수), 오늘의 운세 카탈로그 영역 태깅 실적용(v1.14), 화기격 파괴, 가중치 shadow 실측.

## 2026-09-18 — 오늘의 운세 카탈로그 3차 확장 64→82종 (P4, 데굴님 승인 — docs/17 §22-7 3차)

- **절차**: 공백 진단(OA-7b 9/12~10/11: 상위 5종 64.7%) → 후보표 12종 + 제공 자료 재대조로 6종 추가(평가 누락·지급 지연·가족 일정·
  민원·기기 말썽·찾는 사람 증가) = 18종 승인 → 사전 4종(v1·v2·taxonomy·템플릿) → dict.v1.14·model.v2.2·경계 9/20 → 스냅샷 →
  signature 전수 성립·게이트 리프·감점 12운성 회귀 → 재측정(같은 창 64 vs 82: 상위 5종 42.1→39.4%, headline 종류 45→57).
...
- **불필요 항목 제거(같은 날 데굴님 승인)**: 방향 질문(Q10+direction_question)을 구조 질문 게이트(`is_structural`)에 태워
  ...
  회귀 테스트에 금지 블록 목록 고정.
- **방향 지목 후속(같은 날 실로그)**: '잠잘때 좋은 방향' 뒤 '남쪽은 어때?'가 16종 표의 공부(남쪽 未 천살)로 답함. 원인 = 지목한
  방향을 같은 목적으로 판정하는 재료가 없었음. `detect_asked_direction`·`direction_asked`·0순위 DRILL_DOWN 링크·목적 승계·
  `◆ 질문한 방향` 지지별 판정 블록(정방 3지지/간방 2지지, verdict·중첩 근거)·`ASKED_DIRECTION_DIRECTIVE`. 회귀 4건.
- **방위 각도·간방 정리(같은 날 데굴님 지시)**: 지지=30° 구간(子 정북 345°~15° 중심) 채택(docs/18 §1-8 개정) — 모든 블록에
  '丑 반안살(북북동 15°~45°)' 병기(`BRANCH_COMPASS` SSOT). 질문 방향 16종: 정방=정중앙 대표+양옆 '틀면'+권고, 간방=두 칸 판정+
  틀 방향 권고(verdict 우위, 같으면 동일), 16방위=한 칸. '경계' 표현 제거, 랜드마크 간방 문구도 각도 확인으로. 회귀 갱신.

## 2026-09-21 — 용신 후보 확정 질문 최적화 CAL-P3 (docs/14 §4 개정·§9, 데굴님 승인)

- **결함(실측 1980-11-22)**: 기본 5문항 중 3문항의 사건 기대 극성이 primary 모델 간 전부 같아 답이 점수를 못 가름
  (raw 18:18:18 동점 → prior 로만 결정), 직업 사건 4종 반복, 2014~2017 연속 몰림, 12·15세 해에 결혼·사업 질문, 결과
  화면은 오행 5칸+내부 용어("최적 primary 모델=…gap=0.133").
- **엔진**: `event_discrimination`(모델 간 갈리는 사건 수) 가산 랭킹·미성년 후순위·연속 연도 후순위·판별 사건>0 우선,
  사건 다양화(후보 8→노출 4, 카테고리 cap 2, 반복 회피), 문구·`hint`·`period_range` 재구성, 교운 문항 축약,
  `result_narrator.narrate` → `CalibrationResult.user_summary`(근거 n/m·역할 의미·다음 행동). 채점식 불변.
  실측: 2014·2008·2019·2023·2005(전부 판별 사건>0, 성인, 비연속), 동일 응답에도 primary 점수 −5.5 vs 5.5 로 분리.
- **프런트**: 사건별 결과를 맨 앞, 영역별 체감은 "더 자세히" 접힘, 참고 질문 배지, `hint` 부제, 결과 패널에
  `user_summary` 표시(판정 근거 상세는 접힘), 하단 "판별 문항 x/5".
- 회귀: `test_calibration.py` CAL-P3 3건(판별·성인·비연속·다양화·문구, 동점 해소, 상태별 user_summary).
- **현실 신호 캘리브레이션 CAL-R1(같은 날 데굴님 지시, LIFE_EVENT_INFERENCE §5)**: 연도 선정을 생애 구간
  round-robin + ±1년 후순위 + 최근 우선으로(실측 1999~2006 8년 연속 → 2001·2003·2005·2008·2010·2012·2015·2019·2023·
  2025), 연도별 사건 도메인 cap 2·반복 회피(이직 10/10 → ≤6), `age/band_ko/hint` 부제, 화면은 발생 체크 우선·체감
  접힘·'해당 없음' 설명, 제출 응답 `RealityCalibrationSubmitResult.summary`(전용 페이지 표시). 저장 구조 불변.
